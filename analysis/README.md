...
An RMarkdown document that visualizes the geographic distribution of BGE specimen collection localities across Europe:

**Data Source:**
- `collection_data.tsv`: Contains GPS coordinates (Lat, Lon columns) from BOLD

**Visualization:**
- Map of Europe with country borders
- Collection localities plotted as small red dots with good contrast
- Summary statistics on geographic coverage

The document includes:
- Filtering for valid GPS coordinates
- Interactive map with European country borders
- Summary of localities by country
- Geographic range statistics
...
```r
install.packages(c("readr", "dplyr", "ggplot2", "knitr", "rmarkdown", "tidyr", "lubridate", "scales", "rnaturalearth", "rnaturalearthdata", "sf"))
```
...
# Render the collection map
Rscript -e "rmarkdown::render('analysis/collection_map.Rmd')"
...
1. Open an RMarkdown file (e.g., `analysis/bold_metadata_analysis.Rmd`, `analysis/assembly_parameter_analysis.Rmd`, `analysis/collection_map.Rmd`, `analysis/institution_sequencing_success.Rmd`, `analysis/specimen_age_analysis.Rmd`, or `analysis/taxonomic_validation_analysis.Rmd`) in RStudio
